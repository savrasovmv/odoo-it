    ...
    def sync_obj(self, sync_model_id, obj_id):
        ...
        for field_id in sync_field_id:
            ...
            if field_id.ttype == 'many2one':
                ...
                print("++++ field_id.name", field_id.name)
        ...
        if dist_obj_id:
            _logger.debug("Обновление объекта %s в удаленной БД с id= %s" % (obj_id, dist_obj_id))
            pool_dist.get(sync_model_id.ir_model_id.model).write([dist_obj_id], vals)
        else:
